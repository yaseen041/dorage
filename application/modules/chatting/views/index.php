				...
				<div class="panel panel-default">
					<div class="panel-heading top-bar">
						<div class="col-md-8 col-xs-8">
							<h3 class="panel-title"><span class="glyphicon glyphicon-comment"></span> Chat - <?php echo $chat_user['name']; ?></h3>
						</div>
					...
					<div class="panel-footer">
						<div class="input-group">
							...
							<span class="input-group-btn">
								<button class="btn btn-primary btn-sm btnSendMessage" id="btn-chat" data-chatwith="<?php echo $chat_with; ?>" data-post="<?php echo $post; ?>">Send</button>
							</span>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	var chatwithGlobal = $('.btnSendMessage').attr('data-chatwith');
	var postGlobal = $('.btnSendMessage').attr('data-post');
	setInterval(function(){
		getMessagesAjax(postGlobal, chatwithGlobal);
	}, 10000);
	$(document).on('click','.btnSendMessage', function(){
		...
		$.ajax({
			...
			data:"message="+message+"&chatwith="+chatwith+"&post="+post,
			type:"post",
			success:function(output){
				...
				getMessages(post, chatwith);
			}
		});
	});
	function getMessages(post_id, user_id){
		$.ajax({
			...
			dataType:"json",
			success:function(output){
				$('.msg_container_base').html(output);
				$(".msg_container_base").stop().animate({ scrollTop: $(".msg_container_base")[0].scrollHeight}, 500);
			}
		});
	}

	function getMessagesAjax(post_id, user_id){
		$.ajax({
			url:"<?php echo base_url(); ?>chatting/getMessage",
			data:"post="+post_id+"&user="+user_id,
			type:"post",
